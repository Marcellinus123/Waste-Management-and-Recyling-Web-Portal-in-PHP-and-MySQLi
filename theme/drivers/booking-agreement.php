...
if (!isset($_SESSION['loggedin']) || $_SESSION['usertype'] !== 'waste_driver') {
...
// Get bookings assigned to this driver that need signature
try {
    $stmt = $pdo->prepare("SELECT b.* 
                          FROM bookings b
                          JOIN vehicles v ON b.vehicle_type = v.vehicle_number
                          LEFT JOIN booking_agreements ba ON ba.booking_id = b.booking_code
                          WHERE v.driver_id = ?
                          AND b.status = 'completed'
                          AND (ba.booking_id IS NULL OR ba.waste_driver_sign IS NULL)
    ...
    try {
        ...
        if ($selectedBooking) {
            ...
            if ($agreement) {
                ...
                $alreadySigned = !empty($agreement['waste_driver_sign']);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['sign_agreement'])) {
    ...
    try {
        ...
        // Verify the booking is assigned to this driver and completed
        $stmt = $pdo->prepare("SELECT b.* FROM bookings b
                              JOIN vehicles v ON b.vehicle_type = v.vehicle_number
                              WHERE b.booking_code = ? 
                              AND v.driver_id = ?
        ...
        if (!$booking) {
            throw new Exception("Invalid booking or not assigned to you");
        }
        ...
        if ($agreementExists) {
            // Update existing agreement with driver signature
            if (!empty($agreementExists['waste_driver_sign'])) {
                throw new Exception("You have already signed this agreement");
            }
            
            $stmt = $pdo->prepare("UPDATE booking_agreements
                                 SET waste_driver_sign = 1,
                                     date_driver_sign = NOW()
                                 WHERE booking_id = ?");
            $stmt->execute([$bookingId]);
        } else {
            // Create new agreement with driver signature
            $stmt = $pdo->prepare("INSERT INTO booking_agreements
                                 (booking_id, waste_driver_sign, date_driver_sign, status)
                        ...
                        <div class="tile-body">
                                    ...
                                    <table class="table table-bordered">
                                        <tbody>
                                            ...
                                            <tr>
                                                ...
                                                <td><?= htmlspecialchars($selectedBooking['estimated_weight']) ?></td>
                                            </tr>
                                        </tbody>
                                    </table>
                                </div>
                                    ...
                                    <form method="POST" id="signatureForm">
                                        ...
                                        <div class="card border-primary">
                                            <div class="card-header bg-primary text-white">
                                                <h5 class="card-title mb-0">Service Completion Confirmation</h5>
                                            </div>
                                            <div class="card-body">
                                                <p>By signing this agreement, you confirm that:</p>
                                                <ol>
                                                    <li>You completed the service as scheduled</li>
                                                    <li>The service was performed according to requirements</li>
                                                    <li>The correct vehicle and equipment were used</li>
                                                    <li>Any issues were reported to the customer</li>
                                                    <li>The weight estimate was accurate</li>
                                                </ol>
                            ...
                            <?php elseif ($alreadySigned): ?>
                                <div class="alert alert-success mt-4">
                                    ...
                                    <?= date('F j, Y \a\t g:i a', strtotime($agreement['date_driver_sign'])) ?>
                                </div>
                            <?php endif; ?>
                        </div>
                    </div>
